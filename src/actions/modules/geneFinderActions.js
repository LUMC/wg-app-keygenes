...
export const getGeneSuggestions = (searchTerm) => async (dispatch) =>{
    const result = await SascWebApi.get(`/items/gene?limit=5?filter[symbol][rlike]=${searchTerm}%&filter[symbol][logical]=or&filter[symbol][rlike]=${searchTerm}%&filter[ensg][logical]=or&filter[ensg][rlike]=${searchTerm}%`);
